import type { ClanLore, SiblingLore } from '../lib/api'
import { CHRONICLE_FRAGMENTS } from './chronicles'
...
export const clanLore: ClanLore = {
  clan: 'Clã Retsu',
...
const canonicalSiblings: SiblingLore[] = [
  {
    id: 'haruki',
    order: 1,
    name: 'Haruki Retsu',
...
// Metadados e citações preservam as crônicas; os fragmentos ampliam suas memórias.
export const siblings: SiblingLore[] = canonicalSiblings.map((sibling) => ({
  ...sibling,
  lore: [...sibling.lore, ...(CHRONICLE_FRAGMENTS[sibling.id] ?? [])],
}))
